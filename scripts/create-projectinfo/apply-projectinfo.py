...
logger = logging.getLogger('apply-projectinfo')
set_logger(logger)
...
class ApplyProjectinfo(object):
    ...
    * Apply project with input projectinfo tarball
    ...
    def extract_tarball(self):
        ...
        try:
            run_cmd(['tar', '-x', '-f', self.tarball], cwd=self.apply_path, logger=logger)
    ...
    def setup_project(self):
        try:
           ...
           topcommit = run_cmd(['git', 'rev-parse', 'HEAD'], cwd=wrlinux_x_path, logger=logger)
