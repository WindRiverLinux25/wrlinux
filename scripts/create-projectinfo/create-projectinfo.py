...
import glob
import json
from utils import run_cmd, set_logger

logger = logging.getLogger('create-projectinfo')
set_logger(logger)
...
def get_bitbake_var(var, recipe=None):
    ...
    output = run_cmd(cmd, logger=logger)
...
class CreateProjectinfo(object):
    ...
    * Create project info tarball, may include:
    ...
    def remove_sensitive_info(self, file, project_path = True, functions = False, wrlinux_x_path = False, username = False, hostname = False):
        if project_path:
            run_cmd(['sed', '-i', '-e', 's#%s#/PROJECT_TOPDIR#g' % self.project_topdir, file], logger=logger)
      
        if functions:
            remove_all_functions(file)
        
        if wrlinux_x_path:
            run_cmd(['sed', '-i', '-e', 's#.*wrlinux-x/EULA.*#/WRLINUX-X_TOPDIR/wrlinux-x/EULA#g', file], logger=logger)
    
        if username:
            uname = run_cmd(['whoami'], logger=logger).strip()
            run_cmd(['sed', '-i', '-e', 's#%s#WHOAMI#g' % uname, file], logger=logger)
   
        if hostname:
            hname = run_cmd(['hostname'], logger=logger).strip()
            run_cmd(['sed', '-i', '-e', 's#%s#HOSTNAME#g' % hname, file], logger=logger)
    ...
    def create_projectinfo_tarball(self):
        ...
        run_cmd(['tar', '-cJf', tarball_name, dir_name], cwd=parent_dir, logger=logger)
