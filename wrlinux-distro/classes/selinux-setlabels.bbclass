...
python selinux_setlabels_handler() {
    if not e.data or 'selinux' not in e.data.getVar('DISTRO_FEATURES').split():
        return

    if e.data.getVar('FIRST_BOOT_RELABEL') == '1':
        return

    e.data.appendVar('IMAGE_PREPROCESS_COMMAND', ' selinux_setlabels; ')
    e.data.appendVarFlag('do_image', 'depends', ' policycoreutils-native:do_populate_sysroot')
}
